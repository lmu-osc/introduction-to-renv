...
For example, different versions of the same package might have important differences between how the function `pkg::foo()` works, but R needs to know which version of `foo()` to use when you call it. By storing packages in separate libraries, R can keep track of which version of `foo()` you want to use. These differences between versions happen more often than you'd expect, and should be logged in the package's `NEWS` file or in the package's documentation. Check out this example of "breaking changes" [from the {ggplot2} 3.5.0 package release](https://ggplot2.tidyverse.org/news/index.html#breaking-changes-3-5-0) to get a sense of how behavior of functions can change over time.
