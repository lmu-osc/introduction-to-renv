...
The downside to this approach is that packages are not cached, but rather a temporary library is created every time the `use()` function is employed. For a handful of smaller projects this won't be an issue, but using this approach consistently can noticeably consume disk space over time. This is a trade-off that you will need to consider when deciding whether to use `renv::embed()` or `renv::use()`.
